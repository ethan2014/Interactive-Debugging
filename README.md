...
* 4.a) What would you expect grid to contain before the bug starts hopping? What does it actually contain at this point?  I would expect grid to contain an array of the char ' ' and a few 8's.  It actually contains a weird 'I' character and a few other strange characters.
...
* 4.b) Based on your observations so far, which of the three parts of the program (setup, making the triangle, printing the triangle) must contain our first bug?  It must be the setting up part because the 'I' characters that were put there in the setting up phase are still there after we filled in the triangle.
...
* 6.a) sideLength*sideLength = 40000, so how many times would you expect to hit the line “grid[i] = ‘ ‘” if you are just using “step over” repeatedly?  40000 times.
* 6.b) Try hitting “step over” repeatedly, and see if it does what you expected.  It jumps right out of the for loop without iterating through the loop at all
* 6.c) What is the bug?  There is a ';' at the end of the for loop before its open '{'
...
* 8.a) What should grid look like at the end of the first stage? What does it actually look like?  Nothing but spaces, which is what it looks like.
* 8.b) What should grid look like at the end of the second stage? What does it actually look like?  Given the output, it should look like large strips of '8's, which is not what it looks like.
* 8.c) Which section is the current bug in?  Because of this, the error is in the printing part.
...
* 9.a) What is the bug in the printing loop?  In the body of the second for loop, we were accessing the grid array at index `grid[i * sideLength + i]` which should have been `grid[i * sideLength + j]`.
...
* 11.a) Where is the bug, and how should you fix it?  It is on line 79 `double p3x = sideLength - 1;`.  To fix this, change it to `double p3x = sideLength / 2;`.
* 11.b) If I had not commented my code, how would you have discovered which variable to fix? What if I had used crazy variable names like “fooa, foob, fooc, food, fooe, foof” instead of “p1x, p1y, p2x, p2y, p3x, p3y”?  I would have noticed that you were using those three variables as the three corners of the triangle by looking in the `switch` in the filling in part.  After realizing this, I would know that the top of an equilateral triangle is not on the right side, but in the middle.
...
* 12.a) What is still wrong with the output?  The left sides of each triangle are not being printed.
...
* 13.a) Which portion of the code do you think is causing the problem? If you aren’t sure, you can try repeating step 4, as well as looking over the code.  The filling in part of the code is what is cauing this problem, because it is being initialized with ' 's properly now and it is being printed properly, so the filling in part is the only part left that could cause a problem.
...
* 14.a) What was the bug?  The bug was that there was no `break;` in case 0 of the 'switch' statement, causing `curx` and `cury` to be overwritten by case 1.
